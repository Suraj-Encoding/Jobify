...
public class JobService {
    ...
    public String createJob(String clerkUserId, JobRequest.JobData jobData) {
        ...
        Job job = Job.builder()
                ...
                .salary(getValueOrNull(jobData.getSalary()))
                .company(companyName.trim())
                .companyLogo(recruiter.getCompanyLogo())
                .companyWebsite(recruiter.getCompanyWebsite())
                .type(getValueOrNull(jobData.getType()) != null ? getValueOrNull(jobData.getType()) : "FULL_TIME")
                .experience(getValueOrNull(jobData.getExperience()))
                .skills(getValueOrNull(jobData.getSkills()))
                .requirements(getValueOrNull(jobData.getRequirements()))
                .benefits(getValueOrNull(jobData.getBenefits()))
                .deadline(getValueOrNull(jobData.getDeadline()))
                .maxApplications(jobData.getMaxApplications() != null && jobData.getMaxApplications() > 0 ? jobData.getMaxApplications() : null)
                .applicationCount(0)
                .createdAt(now)
                .build();
